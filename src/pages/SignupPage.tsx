...
import { Video, Mail, Lock, ArrowRight, ShieldCheck, UserPlus } from 'lucide-react';
...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
      {/* Background Glow */}
      <div className="absolute bottom-0 right-0 w-full h-full bg-gradient-to-tl from-orange-500/10 via-transparent to-transparent pointer-events-none" />
      ...
      >
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h1 className="text-3xl font-black tracking-tight mb-2">ÚNETE AHORA</h1>
          <p className="text-white/40 text-sm">Crea tu cuenta para guardar grabaciones ilimitadas.</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-orange-500 text-black py-4 rounded-2xl font-bold hover:bg-orange-400 transition-all flex items-center justify-center gap-2 group"
          >
        ...
        <p className="mt-8 text-center text-sm text-white/40">
          ¿Ya tienes cuenta? <Link to="/login" className="text-white font-bold hover:text-orange-500 transition-colors">Inicia sesión</Link>
        </p>
      </motion.div>
    </div>
  );
